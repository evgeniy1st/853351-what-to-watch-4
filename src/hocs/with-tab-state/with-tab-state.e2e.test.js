...
import WithTabState from "./with-tab-state.jsx";

Enzyme.configure({
  adapter: new Adapter(),
});

it(`Change active class `, () => {
  const film = {
  ...
  const tabsTitle = main.find(`movie-nav__item`);

  tabsTitle.map((it) => {
    it.simulate(`click`, {preventDefault() { }});
    expect(it.class).toBe(`movie-nav__item movie-nav__item--active`);
  });
});
